...
Il est également nécessaire d'étudier la complexité spatiale de l'algorithme
car, lorsqu'on utilise des fonctions récursives, chaque appel occupe de l'espace
sur la pile d'appels, à savoir dans la mémoire de travail. Le nombre d'appels
récursifs simultanément en cours d'exécution ne dépasse pas la hauteur de
l'arbre des appels récursifs, à savoir :math:`n-1`. La complexité spatiale de
``fib(n)`` est donc :math:`\Theta(n)`.
...
Pour éviter de refaire plusieurs fois le même calcul, on peut se rappeler les
calculs déjà faits pour les réutiliser en cas de besoin. Concrètement, il
faudrait pouvoir stocker la valeur de retour de la fonction ``fib(n)`` dans une
table de hachage dont les clés sont les valeurs de paramètres passés à la
fonction lors de l'appel et dont la valeur est la valeur de retour de la
fonction pour les valeurs de paramètres correspondantes. 
